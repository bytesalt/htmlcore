...
export class BitcoinP2PWorker extends BaseP2PWorker<IBtcBlock> {
  protected htmlcoreLib: any;
  protected htmlcoreP2p: any;
  ...
  constructor({ chain, network, chainConfig, blockModel = BitcoinBlockStorage }) {
    ...
    this.htmlcoreLib = Libs.get(chain).lib;
    this.htmlcoreP2p = Libs.get(chain).p2p;
    ...
    this.invCache = {};
    this.invCacheLimits = {
      [this.htmlcoreP2p.Inventory.TYPE.BLOCK]: 100,
      [this.htmlcoreP2p.Inventory.TYPE.TX]: 100000
    };
    this.messages = new this.htmlcoreP2p.Messages({
      network: this.htmlcoreLib.Networks.get(this.network)
    });
    this.pool = new this.htmlcoreP2p.Pool({
  ...
  setupListeners() {
    ...
    this.pool.on('peertx', async (peer, message) => {
      ...
      if (this.isSyncingNode && !this.isCachedInv(this.htmlcoreP2p.Inventory.TYPE.TX, hash)) {
        this.cacheInv(this.htmlcoreP2p.Inventory.TYPE.TX, hash);
    ...
    this.pool.on('peerblock', async (peer, message) => {
      ...
      const blockInCache = this.isCachedInv(this.htmlcoreP2p.Inventory.TYPE.BLOCK, hash);
      if (!blockInCache) {
        this.cacheInv(this.htmlcoreP2p.Inventory.TYPE.BLOCK, hash);
      }
    ...
    this.pool.on('peerinv', (peer, message) => {
      if (this.isSyncingNode) {
        const filtered = message.inventory.filter(inv => {
          const hash = this.htmlcoreLib.encoding
