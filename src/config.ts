...
function findConfig(): ConfigType | undefined {
  ...
  const configFileName = 'htmlcore.config.json';
  let htmlcoreConfigPaths = [
  ...
  if (overrideConfig) {
    htmlcoreConfigPaths.unshift(overrideConfig);
  }
  // No config specified. Search home, htmlcore and cur directory
  for (let path of htmlcoreConfigPaths) {
    if (!foundConfig) {
      try {
        const expanded = path[0] === '~' ? path.replace('~', homedir()) : path;
        const htmlcoreConfig = require(expanded) as { htmlcoreNode: ConfigType };
        foundConfig = htmlcoreConfig.htmlcoreNode;
      } catch (e) {
...
const Config = function(): ConfigType {
  let config: ConfigType = {
    ...
    dbName: process.env.DB_NAME || 'htmlcore',
