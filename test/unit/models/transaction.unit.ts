...
const htmlcoreLib = require('htmlcore-lib');
...
describe('Transaction Model', function() {
  ...
  it('should stream all the mint operations', async () => {
    const tx = htmlcoreLib.Transaction(TransactionFixture.transaction) as Bitcoin.Transaction;
    let batches = 0;
  ...
  describe('Wallet Tagging', async () => {
    const tx = htmlcoreLib.Transaction(TransactionFixture.transaction) as TaggedBitcoinTx;
    const CURRENT_HEIGHT = 8534;
    ...
    it('should tag the transaction ops, and calculate the fee', async () => {
      function getCoinForInput(i: Bitcoin.Transaction.Input) {
        ...
        const inputTx = new htmlcoreLib.Transaction(fixtureInput) as Bitcoin.Transaction;
